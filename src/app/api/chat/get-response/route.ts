...
export async function POST(req: NextRequest) {
  try {
    ...
    let reply: string;
    if (image && message) {
      reply = await queryTextWithImage(
        message,
        image,
    ...
    if (userEmbedding && userEmbedding.length > 0) {
      ...
      await prisma.$executeRaw`
        ...
        VALUES (${activeChatId}, 'user', ${
        message || "[Image]"
      }, ${dbImagePath}, ${`[${userEmbedding.join(",")}]`}::vector, NOW())
      `;
    } else {
    ...
    if (assistantEmbedding && assistantEmbedding.length > 0) {
      await prisma.$executeRaw`
        ...
        VALUES (${activeChatId}, 'assistant', ${reply}, ${`[${assistantEmbedding.join(
        ","
      )}]`}::vector, NOW())
      `;
    } else {
